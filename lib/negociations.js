...
export async function membresDeLaNegociation(negociationId) {
  return sql()`
    select u.id as utilisateur_id, u.nom, u.email, u.dernier_vu, u.photo_url, m.role, m.actif, m.titre_autre,
...
// la Boîte noire. Renvoie false si rien n'a été touché (pas autorisé, ou
// déjà supprimé) -- l'appelant distingue ainsi un vrai refus d'un no-op.
export async function supprimerMessage(id, utilisateurId, estModerateur) {
  const rows = estModerateur
    ? await sql()`update messages set supprime_le = now(), supprime_par_id = ${utilisateurId} where id = ${id} and supprime_le is null returning id`
    : await sql()`update messages set supprime_le = now(), supprime_par_id = ${utilisateurId} where id = ${id} and auteur_id = ${utilisateurId} and supprime_le is null returning id`;
  return rows.length > 0;
}

export async function supprimerMessageClient(id, utilisateurId, estModerateur) {
  const rows = estModerateur
    ? await sql()`update messages_client set supprime_le = now(), supprime_par_id = ${utilisateurId} where id = ${id} and supprime_le is null returning id`
    : await sql()`update messages_client set supprime_le = now(), supprime_par_id = ${utilisateurId} where id = ${id} and auteur_id = ${utilisateurId} and supprime_le is null returning id`;
  return rows.length > 0;
}
